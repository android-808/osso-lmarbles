...
#endif
    snprintf(prf_pth, 255, "%s%s", getenv("HOME"), "/.lmarbles_profile");
}
        ...
        else
        {
...
#ifdef ASCII
            F_GetE(f, str, F_VAL);
            F_ValToInt(str, &p_num);
#else
            fread((void *) &p_num, sizeof(int), 1, f);
#endif
            if (p_num <= 0)
            {
                ULOG_WARN("WARNING: bad profile counter: %i\n", p_num);
                Prf_Crt("Michael");
                not_f = 1;
            }
            else
            {
                while (p_num--)
                {
                    ...
                    /* name */
                    F_GetE(f, p->nm, F_VAL);
                    p->nm[strlen(p->nm) - 1] = 0;
                    /* levels played */
                    F_GetE(f, str, F_VAL);
                    F_ValToInt(str, &p->lvls);
                    /* score */
                    F_GetE(f, str, F_VAL);
                    F_ValToInt(str, &p->scr);
                    /* percentage */
                    F_GetE(f, str, F_VAL);
                    F_ValToFloat(str, &p->pct);
                    /* number of levelsets */
                    F_GetE(f, str, F_VAL);
                    F_ValToInt(str, &s_num);
#else
                    /* name */
...
#ifdef ASCII
                            F_GetE(f, st->nm, F_VAL);
                            st->nm[strlen(st->nm) - 1] = 0;
                            F_GetE(f, str, F_VAL);
                            F_ValToInt(str, &st->num);
                            F_GetE(f, str, F_VAL);
                            F_ValToInt(str, &st->l_num);
                            F_GetE(f, str, F_VAL);
                            F_ValToInt(str, &st->c_num);
                            for (i = 0; i < st->c_num; i++)
                            {
                                F_GetE(f, str, F_VAL);
                                F_ValToChar(str, &st->c_opn[i]);
                            }
                            for (i = 0; i < st->num; i++)
                            {
                                F_GetE(f, str, F_VAL);
                                F_ValToChar(str, &st->cmp[i]);
                            }
#else
...
Prf_Sv()
{
    ...
    ULOG_DEBUG("saving profiles... ");
    if ((f = fopen(prf_pth, "w")) == 0)
    {
        ULOG_WARN("WARNING: no write access to %s\n", prf_pth);
    }
    else
    {
        ...
        while (e != &prfs.tl)
        {
            ...
            F_WrtE(f, p->nm);
            /* levels played */
            F_IntToStr(str, p->lvls);
            F_WrtE(f, str);
            /* score */
            F_IntToStr(str, p->scr);
            F_WrtE(f, str);
            /* percentage */
            F_FloatToStr(str, p->pct);
            F_WrtE(f, str);
            /* save number of levelsets */
            F_IntToStr(str, p->sts.cntr);
            F_WrtE(f, str);
#else
            /* save name -- 12 chars */
            ...
            while (le != &p->sts.tl)
            {
                ...
                F_IntToStr(str, st->num);
                F_WrtE(f, str);
                /* save level peer chapter number */
                F_IntToStr(str, st->l_num);
                F_WrtE(f, str);
                /* save chapter number */
                F_IntToStr(str, st->c_num);
                F_WrtE(f, str);
                /* save chapter open flags */
                for (i = 0; i < st->c_num; i++)
                {
                    F_IntToStr(str, st->c_opn[i]);
                    F_WrtE(f, str);
                }
                /* save flags */
                for (i = 0; i < st->num; i++)
                {
                    F_IntToStr(str, st->cmp[i]);
                    F_WrtE(f, str);
                }
#else
                /* save name -- 32 chars */
...
Prf_Crt(char *nm)
{
    ...
    while (e != &prfs.tl)
    {
        ...
        {
            ULOG_WARN("WARNING: profile '%s' already exists\n", nm);
            return;
        }
        e = e->n;
    }
    ...
    while (e != &p->sts.tl)
    {
        ...
        if (!strcmp(s->nm, l_st->nm))
        {
            if (l_st->c_num != s->c_num || l_st->l_num != s->l_num)
            {
                ...
                ULOG_WARN
                    ("WARNING: profile '%s': set info '%s' seems to be out of date\n",
                     p->nm, l_st->nm);
            }
            return s;
        }
...
/* 
 * update Profile p's score and info s is rem_time / max_time of that level */
void
Prf_Upd(Prf * p, SInf * inf, int l_id, float pct, int scr)
{
    float new_p;
#ifdef DEBUG
    float old_p = p->pct;
    ULOG_DEBUG("level %i...", l_id);
#endif
    if (!inf->cmp[l_id])
    {
        /* mark as completed */
        ...
        p->scr += scr;
#ifdef DEBUG
        ULOG_DEBUG("marked as completed\n");
        ULOG_DEBUG("added %4.2f: percentage changed from %4.2f to %4.2f\n",
                   pct, old_p, p->pct);
        ULOG_DEBUG("score added: %i\n", scr);
#endif
    }
#ifdef DEBUG
    else
        ULOG_DEBUG("already finished\n");
#endif
}
