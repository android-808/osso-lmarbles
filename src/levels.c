...
/* 
 * count and create a list with all loadable filenames found in
 * SRC_DIR/levels */
void
L_CrtLst()
{
...
#endif
    sprintf(d_nm, "%s", LMARBLESLEVELSDIR);
    ...
    ULOG_DEBUG("searching for level sets...\n");
    ...
    if (ls_n == 0)
    {
        ULOG_ERR("ERROR: '%s' seems to be empty\n", d_nm);
        closedir(dir);
        exit(1);
    }
    else
        ULOG_DEBUG("...total of %i\n", ls_n);
...
void
L_StMpT(MapT * tl, int m, int t, int id)
{
    tl->m = m;
    tl->t = t;
    tl->id = id;
...
L_LdSt(FILE * f)
{
    ...
    {
        ULOG_ERR("ERROR: line %i: '<info>' expected\n", f_ln);
        free(st);
        return 0;
    }
    // levels per chapter //
    ...
    {
        ULOG_ERR("ERROR: line %i: 'levels' expected\n", f_ln);
        free(st);
        return 0;
    }
    st->l_num = atoi(val);
    ...
    {
        ULOG_ERR("ERROR: line %i: 'chapters' expected\n", f_ln);
        free(st);
        return 0;
    }
    st->c_num = atoi(val);
    ...
    {
        ULOG_ERR("ERROR: line %i: 'limit' expected\n", f_ln);
        free(st);
        return 0;
    }
    ...
    for (i = 0; i < st->c_num; i++)
    {
        ...
        {
            ULOG_ERR("ERROR: line %i: '<chapter>' expected\n", f_ln);
            goto failure;
        }
        // name //
        F_GetE(f, str, F_VAL);
        ...
        {
            ULOG_ERR("ERROR: line %i: 'name' expected\n", f_ln);
            goto failure;
        }
        // author //
        F_GetE(f, str, F_VAL);
        ...
        {
            ULOG_ERR("ERROR: line %i: 'author' expected\n", f_ln);
            goto failure;
        }
        // gset //
        F_GetE(f, str, F_VAL);
        ...
        {
            ULOG_ERR("ERROR: line %i: 'gfx_set' expected\n", f_ln);
            goto failure;
        }
        // open for play ? //
        F_GetE(f, str, F_VAL);
        ...
        {
            ULOG_ERR("ERROR: line %i: 'open' expected\n", f_ln);
            goto failure;
        }
        ...
        for (j = 0; j < st->l_num; j++)
        {
            ...
            {
                ULOG_ERR("ERROR: line %i: '<level>' expected\n", f_ln);
                goto failure;
            }
            // time //
            F_GetE(f, str, F_VAL);
            ...
            {
                ULOG_ERR("ERROR: line %i: 'map_w' expected\n", f_ln);
                goto failure;
            }
            if ((st->ch[i].lvls[j].m_w = atoi(val)) > L_MAX_W
                || atoi(val) < L_MIN_W)
            {
                ULOG_ERR("ERROR: line %i: 'map_w' out of range (%i-%i): %i\n",
            ...
            {
                ULOG_ERR("ERROR: line %i: 'map_h' expected\n", f_ln);
                goto failure;
            }
            if ((st->ch[i].lvls[j].m_h = atoi(val)) > L_MAX_H
                || atoi(val) < L_MIN_H)
            {
                ULOG_ERR("ERROR: line %i: 'map_w' out of range (%i-%i): %i\n",
            ...
            {
                ULOG_ERR("ERROR: line %i: 'fig_w' expected\n", f_ln);
                goto failure;
            }
            st->ch[i].lvls[j].f_w = atoi(val);
            if ((st->ch[i].lvls[j].f_w = atoi(val)) > F_MAX_W
                || atoi(val) < F_MIN_W)
            {
                ULOG_ERR("ERROR: line %i: 'fig_w' out of range (%i-%i): %i\n",
            ...
            {
                ULOG_ERR("ERROR: line %i: 'fig_h' expected\n", f_ln);
                goto failure;
            }
            if ((st->ch[i].lvls[j].f_h = atoi(val)) > F_MAX_H
                || atoi(val) < F_MIN_H)
            {
                ULOG_ERR("ERROR: line %i: 'fig_h' out of range (%i-%i): %i\n",
            ...
            {
                ULOG_ERR("ERROR: line %i: '<figure>' expected\n", f_ln);
                goto failure;
            }
            // figure //
            for (k = 0; k < st->ch[i].lvls[j].f_h; k++)
            {
                ...
                for (l = 0; l < st->ch[i].lvls[j].f_w; l++)
                {
                    if (str[l] >= '0' && str[l] <= '9')
                        ...
                        ULOG_ERR("WARNING: line %i: bad figure marble '%c'\n",
                                 f_ln, str[l]);
                    else
            ...
            {
                ULOG_ERR("ERROR: line %i: '<map>' expected\n", f_ln);
                goto failure;
            }
            // map
            for (k = 0; k < st->ch[i].lvls[j].m_h; k++)
            {
                ...
                {
                    if (str[l] >= '0' && str[l] <= '9')
                        L_StMpT(&st->ch[i].lvls[j].map[l][k], str[l] - 48,
                                M_FLOOR, 0);
                    else
                        switch (str[l])
                        {
                            ...
                            case 'b':
                            case 'c':
                                L_StMpT(&st->ch[i].lvls[j].map[l][k], -1,
                                        M_WALL, str[l] - 97);
                                break;
                            case ' ':
                                L_StMpT(&st->ch[i].lvls[j].map[l][k], -1,
...
L_Ini(int c, int l, char flags)
{
    ...
    if (flags & L_DATA)
    {
        ...
        memcpy(gm.c_lvl, &gm.c_l_st->ch[gm.c_ch].lvls[gm.c_l_id],
               sizeof(Lvl));
        ...
        while (e != &gm.g_sts.tl)
        {
            if (!strncmp(nm, ((GSet *) e->d)->nm, strlen(nm))
                && ((GSet *) e->d)->ok)
            {
                gm.c_g_st = (GSet *) e->d;
                gst_ok = 1;
        ...
        {
            ULOG_WARN
                ("WARNING: unknown or unuseable gfx set '%s';\nsearching for a good one...\n",
                 nm);
            ...
            if (!gst_ok)
            {
                ULOG_ERR("ERROR: no good gfx set found...\n");
                exit(1);
            }
        }
        gm.l_x =
            (gm.scr_w - gm.brd_w -
             gm.c_l_st->ch[gm.c_ch].lvls[gm.c_l_id].m_w * gm.t_w) / 2;
        gm.l_y =
            (gm.scr_h -
             gm.c_l_st->ch[gm.c_ch].lvls[gm.c_l_id].m_h * gm.t_h) / 2;
        ...
        {
            /* gm.c_lvl->tm containts the move limit. this is modified
             * according to the difficulty levels */
            switch (cfg.diff)
            {
                case DIFF_EASY:
                    mv_mod =
                        (int) ceil((float) gm.c_lvl->tm * 0.2 *
                                   (5 - gm.c_ch));
                    gm.c_lvl->tm += mv_mod;
                    if (gm.c_lvl->tm % 2)
                        gm.c_lvl->tm++;
                ...
                case DIFF_NORMAL:
                    mv_mod =
                        (int) ceil((float) gm.c_lvl->tm * 0.1 *
                                   (5 - gm.c_ch));
                    gm.c_lvl->tm += mv_mod;
                    if (gm.c_lvl->tm % 2)
                        gm.c_lvl->tm++;
                ...
                case DIFF_HARD:
                    mv_mod =
                        (int) ceil((float) gm.c_lvl->tm * 0.05 *
                                   (5 - gm.c_ch));
                    gm.c_lvl->tm += mv_mod;
                    if (gm.c_lvl->tm % 2)
                        gm.c_lvl->tm++;
    ...
    if (flags & L_GFX)
    {
        ...
        if (gm.c_g_st->s_bkgd)
        {
            for (j = 0; j < gm.scr_w - gm.brd_w; j += gm.c_g_st->s_bkgd->w)
                ...
                {
                    D_DST(gm.s_bkgd, j, k, gm.c_g_st->s_bkgd->w,
                          gm.c_g_st->s_bkgd->h);
        ...
        // add board
        D_DST(gm.s_bkgd, gm.s_bkgd->w - gm.s_brd->w, 0, gm.s_brd->w,
              gm.s_brd->h);
        D_SRC(gm.s_brd, 0, 0);
        SS_Blt();
        ...
        if (gm.ms_us)
        {
            D_DST(gm.s_bkgd, gm.s_bkgd->w - gm.s_brd->w + 56, 110,
                  gm.s_undo->w, gm.s_undo->h);
            D_SRC(gm.s_undo, 0, 0);
        }
        else
        {
            D_DST(gm.s_bkgd, gm.s_bkgd->w - gm.s_brd->w + 56, 110,
                  gm.s_undod->w, gm.s_undod->h);
            D_SRC(gm.s_undod, 0, 0);
        }
        SS_Blt();
	
	/* Add the overlay button -Sandy */
        D_DST(gm.s_bkgd, gm.s_bkgd->w - gm.s_brd->w + 230, 0,
              gm.s_ret->w, gm.s_ret->h);
        D_SRC(gm.s_ret, 0, 0);
        SS_Blt();

        // add static level gfx
        lp = gm.c_lvl;          // &gm.c_l_st->ch[gm.c_ch].lvls[gm.c_l_id];
        for (i = 0; i < lp->m_w; i++)
            for (j = 0; j < lp->m_h; j++)
                L_DrwMpTl(i, j);
        ...
        for (i = 0; i < lp->m_w; i++)
            for (j = 0; j < lp->m_h; j++)
                if (lp->map[i][j].m != -1)
                {
                    D_DST(gm.s_bkgd, gm.l_x + i * gm.t_w, gm.l_y + j * gm.t_h,
                          gm.t_w, gm.t_h);
        ...
        for (i = 0; i < lp->f_w; i++)
            for (j = 0; j < lp->f_h; j++)
                if (lp->fgr[i][j] != -1)
                {
                    D_DST(gm.s_bkgd, px + i * gm.f_w, py + j * gm.f_h, gm.f_w,
                          gm.f_h);
                    D_SRC(gm.s_fig, 0, lp->fgr[i][j] * gm.f_h);
                    SS_Blt();
                }

        G_Tmpl(str, 63, _("game_fi_lmarbles_main_score"), gm.c_prf->scr, 0);
        FT_DrwTxtAlgn(gm.ft_fnt, gm.s_bkgd, gm.i_x + gm.b_x, gm.i_y, FT_ALIGN_Y_T, gm.ft_uis, str, COL_MAIN);   // #ffc833
        if (gm.c_prf->scr > gm.hi_scr)
            gm.hi_scr = gm.c_prf->scr;
        G_Tmpl(str, 63, _("game_fi_lmarbles_main_high_score"), gm.hi_scr, 0);
        FT_DrwTxtAlgn(gm.ft_fnt, gm.s_bkgd, gm.i_x + gm.b_x, gm.i_y + gm.i_h, FT_ALIGN_Y_B, gm.ft_uis, str, COL_MAIN);  // #ffc833

        // compute position of first chapter
        gm.c_x = gm.s_x;        // changed
        ...
        for (j = 0; j < gm.c_l_st->c_num; j++)
            ...
            {
                D_DST(gm.s_bkgd, gm.b_x + gm.c_x + i * L_SIZE,
                      gm.c_y + j * L_SIZE, L_SIZE, L_SIZE);
                if (!gm.c_s_inf->c_opn[j])
                {
                    D_SRC(gm.s_lghts, L_RED * L_SIZE, 0);
                }
                else if (gm.c_s_inf->cmp[j * gm.c_l_st->l_num + i])
                {
        ...
        D_DST(gm.s_bkgd, gm.b_x + gm.c_x + l * L_SIZE, gm.c_y + c * L_SIZE,
              L_SIZE, L_SIZE);
...
/* 
 * draw a map tile to gm.s_bkgd with x,y position in the map using current
 * map */
void
L_DrwMpTl(int i, int j)
{
    switch (gm.c_lvl->map[i][j].t)
    {
        case M_WALL:
            ...
            if (i + 1 >= gm.c_lvl->m_w
                || gm.c_lvl->map[i + 1][j].t == M_EMPTY)
            {

                D_DST(gm.s_bkgd, gm.l_x + (i + 1) * gm.t_w,
                      gm.l_y + j * gm.t_h, 1, gm.t_h + 1);
                SS_Fill(0x0);

            }
            // add wall shadow on bottom ? //
            if (j + 1 >= gm.c_lvl->m_h
                || gm.c_lvl->map[i][j + 1].t == M_EMPTY)
            {

                D_DST(gm.s_bkgd, gm.l_x + i * gm.t_w,
                      gm.l_y + (j + 1) * gm.t_h, gm.t_w + 1, 1);
                SS_Fill(0x0);
            ...
            if ((i == 0)
                || ((i - 1 >= gm.c_lvl->m_w)
                    || gm.c_lvl->map[i - 1][j].t == M_EMPTY))
            {
                D_DST(gm.s_bkgd, gm.l_x + i * gm.t_w - 1, gm.l_y + j * gm.t_h,
                      1, gm.t_h + 1);
                SS_Fill(0x0);
            ...
            if ((j == 0)
                || ((j - 1 >= gm.c_lvl->m_h)
                    || gm.c_lvl->map[i][j - 1].t == M_EMPTY))
            {
                D_DST(gm.s_bkgd, gm.l_x + i * gm.t_w, gm.l_y + j * gm.t_h - 1,
                      gm.t_w + 1, 1);
                SS_Fill(0x0);

            }
            D_DST(gm.s_bkgd, gm.l_x + i * gm.t_w, gm.l_y + j * gm.t_h, gm.t_w,
                  gm.t_h);
        ...
        case M_CRUMBLE:
            ...
            {
                case 0:
                    FT_DrwTxtAlgn(gm.ft_fnt, gm.s_bkgd,
                                  gm.l_x + (i + 1) * gm.t_w - 1,
                                  gm.l_y + (j + 1) * gm.t_h - 1,
                                  FT_ALIGN_X_R | FT_ALIGN_Y_B, 12, "1",
                                  COL_DLG);
                    // SF_Wrt(gm.f_sml, gm.s_bkgd, gm.l_x + (i + 1) * gm.t_w
                    // - 1, gm.l_y + (j + 1) * gm.t_h - 1, "1", 0);
                    break;
                case 1:
                    FT_DrwTxtAlgn(gm.ft_fnt, gm.s_bkgd,
                                  gm.l_x + (i + 1) * gm.t_w - 1,
                                  gm.l_y + (j + 1) * gm.t_h - 1,
                                  FT_ALIGN_X_R | FT_ALIGN_Y_B, 12, "2",
                                  COL_DLG);
                    break;
                case 2:
                    FT_DrwTxtAlgn(gm.ft_fnt, gm.s_bkgd,
                                  gm.l_x + (i + 1) * gm.t_w - 1,
                                  gm.l_y + (j + 1) * gm.t_h - 1,
                                  FT_ALIGN_X_R | FT_ALIGN_Y_B, 12, "3",
                                  COL_DLG);
                    break;
            }
...
L_FndNxt()
{
    ...
    if (gm.c_l_id >= gm.c_l_st->l_num)
    {
        gm.c_l_id = 0;
        gm.c_ch++;
        // check if this chapter can be entered or if this was the last
        // chapter if not take first unsolved level found
        if (gm.c_ch >= gm.c_l_st->c_num || !gm.c_l_st->ch[gm.c_ch].opn)
        {
            for (i = 0; i < gm.c_ch; i++)
                for (j = 0; j < gm.c_l_st->l_num; j++)
                    ...
                    {
                        // not all levels completed jump back to first
                        // unsolved level
                        gm.c_ch = i;
                        gm.c_l_id = j;
                        return 1;
