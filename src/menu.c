 ...
 * set range */
void
ME_StRng(MEnt * e, int *p, int min, int max, int stp)
{
    e->p = p;
    e->min = min;
    e->max = max;
...
MEnt *
ME_CrtSw2(char *n, int *p, char *cap1, char *cap2)
{
    char *sw;
    MEnt *e = ME_Crt();
...
MEnt *
ME_CrtSwX(char *n, int *p, char **caps, int num)
{
    int i;
    char *sw;
    MEnt *e = ME_Crt();
 ...
 * create range */
MEnt *
ME_CrtRng(char *n, int *p, int min, int max, int stp)
{
    MEnt *e = ME_Crt();
...
/* 
 * create an edit min is the current length max the max length and p the char 
 * pointer */
MEnt *
ME_CrtStr(char *n, char *p, int m)
{
    MEnt *e = ME_Crt();
...
ME_Edt(MEnt * e, int c, int u)
{
    ...
    else if (c >= 32 && c < 128 && e->min < e->max)
    {
        // letter
        ((char *) e->p)[e->min++] = u;
    }
...
void
ME_CngSwX(MEnt * e, int *p, char **caps, int num)
{
    int i;
    char *sw;
    ...
    while (e != &m->ent.tl)
    {
        ...
        if (me == m->c_e)
        {
            if (me->a > 0)
            {
                ...
                SF_Wrt(mm.ft_sel, sdl.scr, me->x, me->y, me->str,
                       (int) me->a);
            }
            else
...
void
MM_Ini(int x, int y, int ly, SDL_Surface * ss_bk, SDL_Surface * ss_lg,
       SFnt * fn, SFnt * fs)
{
    mm.ss_bkgd = ss_bk;
    mm.ss_logo = ss_lg;
...
/* 
 * show the menu sets a new video mode if wanted */
void
MM_Shw(int rsz)
{
    if (rsz)
        Sdl_StVdMd(mm.ss_bkgd->w, mm.ss_bkgd->h, 16, SDL_SURFACE_TYPE);
    D_FDST(sdl.scr);
    D_FSRC(mm.ss_bkgd);
    SS_Blt();
    ...
    {
        fprintf(stderr,
                "MM_Shw(): Ups... no current or empty menu defined!\n");
        exit(1);
    }
    mm.c_mn->c_e->a = 0;
    M_Shw(mm.c_mn);
...
{
    if (mm.ss_bkgd)
        SDL_FreeSurface(mm.ss_bkgd);
    if (mm.ss_logo)
        SDL_FreeSurface(mm.ss_logo);
    if (mm.ft_nml)
        SF_Fr(mm.ft_nml);
    if (mm.ft_sel)
        SF_Fr(mm.ft_sel);
...
/* 
 * adjust position of all entries in all menus relative to mm.x | mm.y as
 * centre */
void
MM_AdjP()
{
    ...
    while (m != &mm.mn.tl)
    {
        ...
        while (e != &((Menu *) m->d)->ent.tl)
        {
            ...
            me->x = mm.x;
            me->y =
                (mm.y - (((Menu *) m->d)->ent.cntr * mm.ft_nml->h) / 2) +
                i * mm.ft_nml->h + mm.ft_nml->h / 2;
            i++;
            e = e->n;
        }
        m = m->n;
...
/* 
 * checks if all menus contain at least one entry and if every menu has an
            ...
            switch (e->key.keysym.sym)
            {
                ...
                default:
                    if (mm.c_mn->c_e->t == ME_STR)
                        ME_Edt(mm.c_mn->c_e, e->key.keysym.sym,
...
/* 
 * use an entry submenu: set submenu action: return action range: change pos
 * due to c (inc or dec) str: add or delete char */
int
MM_UseE(int c)
{
    MEnt *me = mm.c_mn->c_e;
...
MM_PrvE()
{
    ...
    while (e != &mm.c_mn->ent.hd && ((MEnt *) e->d)->t == ME_SEP)
        e = e->p;
    if (e != &mm.c_mn->ent.hd)
    {
...
MM_NxtE()
{
    ...
    while (e != &mm.c_mn->ent.tl && ((MEnt *) e->d)->t == ME_SEP)
        e = e->n;
    if (e != &mm.c_mn->ent.tl)
    {
        mm.c_mn->c_e = (MEnt *) e->d;
...
MM_SelE(int x, int y)
{
    ...
    {
        me = (MEnt *) e->d;
        if (me->t != ME_SEP && x >= me->dx && x < me->dx + me->dw
            && y >= me->dy && y < me->dy + me->dh)
        {
            mm.c_mn->c_e = me;
            mm.c_mn->c_e->a = 0;
#ifdef SOUND
